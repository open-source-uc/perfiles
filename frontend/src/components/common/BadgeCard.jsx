...
function BadgeCard({
  name, image, hasAchievement, setIsOpen, width = 'w-40', height = 'h-48', textSize = 'text-m',
}) {
  // eslint-disable-next-line no-shadow
  function buttonClicked(e, name) {
  ...
  return (
    <article className={`${width} ${height} rounded-xl shadow-xl bg-white px-6 flex flex-col justify-center ${hasAchievement ? 'border-indigo-500 border-4' : ''}`}>
      <button type="submit" className="absolute w-6 fill-yellow-500 top-2 left-[8rem]" onClick={(e) => buttonClicked(e, name)}>
